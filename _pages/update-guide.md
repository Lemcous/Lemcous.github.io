...
网站源代码保存在 [Lemcous.github.io 仓库](https://github.com/Lemcous/Lemcous.github.io)。不同内容对应不同文件：

| 要更新的内容 | 对应文件或文件夹 |
| --- | --- |
| 首页介绍、MBTI、学习经历和首页展示区域 | `index.md` |
| 个人简历中的教育、项目、实践与技能 | `_pages/cv.md` |
| 个人作品集 | `_pages/works.md` |
| 网站名称、侧栏简介、邮箱和头像设置 | `_config.yml` |
| 简历、作品集等页面的顶部导航 | `_data/navigation.yml` |
| 首页自定义导航 | `index.md` 中的 `xz-nav` |
| 图片素材 | `images/` |
| PDF、作品文档等附件 | `files/` |
| 本说明页面 | `_pages/update-guide.md` |

**首页应修改根目录的 `index.md`。** 仓库中的 `_pages/about.md` 保留了早期介绍内容，并不是当前首页新版内容的编辑位置。联系方式等信息可能出现在多个页面，修改时应检查首页、简历与侧栏是否一致。
...
```html
<p class="xz-copy">南京大学新闻传播学院新闻与传播硕士研究生。</p>
```
...
**更新简历：** 打开 `_pages/cv.md`，找到“教育经历”“研究与项目”或“实践经历”等对应区域，修改学校、时间、职责和成果。新增经历时，可复制同一区域已有的完整条目，再替换文字，保留成对的标签。

**添加作品：** 先把 PDF 或图片上传到 `files/` 或 `images/`，再编辑 `_pages/works.md`。当前作品集有“作品整理中”的占位区域，可将该区域替换为实际作品。每件作品建议说明名称、时间、背景、个人职责和成果，并附上可访问的链接。
...
如果也希望在首页展示该作品，还需要同步修改 `index.md` 中对应的作品展示区域。
...
替换图片时，可以上传新文件并修改页面引用路径。侧栏头像由 `_config.yml` 中的 `author.avatar` 设置；首页中的图片需在 `index.md` 中单独检查和替换。首页有些图像可能以内嵌 SVG 或图片数据形式保存，这类图像不会随着侧栏头像设置自动更新。
...
其中，`title` 是页面标题，`permalink` 是页面网址路径。每个新页面应使用独立路径。
...
首页使用独立导航，还需在 `index.md` 的 `xz-nav__links` 内补充链接：

```html
<a href="/new-project/">新项目</a>
```
