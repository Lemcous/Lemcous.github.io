...
**“关于我”首页应修改根目录的 `index.md`。** 首页集中展示个人介绍和简历经历；旧的 `/cv/` 地址会跳转至首页。联系方式修改时应检查首页与侧栏是否一致。
...
**更新简历：** 打开 `index.md`，找到“教育经历”“研究与项目”或“实践经历”等对应区域，修改学校、时间、职责和成果。新增经历时，可复制同一区域已有的完整条目，再替换文字，保留成对的标签。
...
替换图片时，可以上传新文件并修改页面引用路径。首页与其他页面的侧栏头像由 `_config.yml` 中的 `author.avatar` 设置。作品正文中的图片需在对应页面文件中单独检查和替换。
